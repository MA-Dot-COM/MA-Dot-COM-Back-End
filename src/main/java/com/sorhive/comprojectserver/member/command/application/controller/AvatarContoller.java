...
public class AvatarContoller {
    ...
    public ResponseEntity<ResponseDto> createAvatar(@RequestHeader String Authorization, @Valid @RequestBody AvatarCreateDto avatarCreateDto) {
        ...
        log.info("[AvatarContoller] avatarCreateDto : " + avatarCreateDto);

        String accessToken = Authorization.substring(7);

        return ResponseEntity.ok().body(new ResponseDto(HttpStatus.OK, "아바타 생성 성공", avatarInfraService.createAvatar(accessToken, avatarCreateDto)));
